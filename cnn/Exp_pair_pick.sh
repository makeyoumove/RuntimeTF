...
if [ $# -lt 4 ] ; then
	echo Usage: $0 [Model_imagenet] [Model_cifar] [Split_imagenet] [Split_cifar]
	exit
fi

echo Split $1 $2 

imagenetlist=(`cat imagenet_list | grep $1`)
cifarlist=(`cat cifar_list | grep $2`)
...
#		iter2=50
		...
		./imagenet_repeat.sh imagenet $model1 $bsize1 $iter1 $3 >& data_cifar10_${model2}_imagenet_${model1}/log_imagenet &
#		./imagenet_repeat.sh imagenet $model1 $bsize1 100 $1 >& data_cifar10_${model2}_imagenet_${model1}/log_imagenet &
		pid=$!
		./test.sh cifar10 $model2 $bsize2 $iter2 $4 >& data_cifar10_${model2}_imagenet_${model1}/log_cifar
